document.addEventListener("DOMContentLoaded", async function() {
    ...
    async function fetchPromptsTranslation() {
        try {
            const response = await fetch('/prompts');
            if (!response.ok) {
                throw new Error('Error fetching prompts translation');
            }
            const data = await response.json();
            promptsTranslation = data.translatedPrompts;
            promptTranslationText.innerText = promptsTranslation[currentPromptIndex];
        } catch (e) {
            console.error('Error fetching prompts translation:', e);
        }
    }
    ...
    async function startRecording(mq) {
        ...
        // recordButton.innerHTML = '<img src="img/wave.gif" alt="recording" style="width: 300px; height: auto;">';
        moveToNewPosition();
        startRecognition();
    ...
    function stopRecording(mq) {
        if (isRecording){
            ...
            // recordButton.innerHTML = '<img src="img/microphone.png" alt="recording" style="width: 175px; height: auto;">';
            moveToNewPosition();
            mediaRecorder.stop();
            isRecording = false;
            stopRecognition();
